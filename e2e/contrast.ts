 ...
 *    `color-mix()`. Every surface this lab uses to state an outcome is one:
 *    `.chip-ok`, `.chip-alarm` and `.chip-warn` (the three verdict chips),
 *    `.vb-ok` and `.vb-fail` (the MAC-diff badges), `.aha` and `.aha-ok` (the
 *    two supporting-copy boxes), `.byte-changed` (the `<mark>` on every byte
 *    that differs), `.pkt-remove:hover` and `.cl-hero-why` are each a
 *    `color-mix(in srgb, …, transparent)` — or, for the hero aside, `in oklab`
 *    — layered over `--surface` or `--surface-2`. axe files all of them under
 *    `incomplete`, which means a violations-only gate measured the contrast of
 *    almost nothing this demo renders as a verdict: not one chip, not one
 *    badge, not one highlighted byte.
 ...
 *    at `.85`. `.pkt-struck` is the near miss: the struck-through entries in
 *    the attacker's lane are the obvious place for an `opacity` fade, and the
 *    stylesheet writes `opacity: 1` explicitly and de-emphasises with
 *    `line-through` plus `--text-dim` instead. This walk is what keeps that
 *    honest if anyone changes it.
 *
 ...
 *    boxes are the `.byteblock` hex regions — every one of the eight byte rows
 *    the MAC diff renders is `overflow-x: auto` around `white-space: nowrap`
 *    content far wider than the box at any width, since a 32-byte transcript
 *    hash is 96 characters — and `.flight-track`, which is `overflow: hidden`
 *    around a packet that travels outside it. Content scrolled or slid past
 ...
 *    marks, both `aria-hidden` and glyph-free; every figure it draws (the three
 *    wire lanes, the flight track, the byte rows, the MAC diff) is HTML. So the
 ...
 * generated content. It iterates ELEMENTS, and a `::before`/`::after` is not
 * one — nor is it one to axe's `color-contrast` rule. On this page that blind
 * spot is genuinely empty, which was checked rather than assumed: the stylesheet
 * declares exactly one pseudo-element, `.flight-track::before`, and its
 * `content` is the empty string. It is a 2px dashed rule standing in for the
 * wire the packet rides on, carries no glyph and no meaning that the three
 * `.flight-tick` labels beside it do not state in words. There is no counter, no
 * status marker, and no `appearance: none` select needing a drawn chevron —
 * every control here is a native radio, checkbox or button. (`--chevron` is
 * defined in both palettes and referenced nowhere; it is a leftover from a
 * select this lab does not have.)
 */
 ...
 *
 * The default is the whole page, and that is what `scan` uses. The narrowed
 * form exists for the one state on this page with a lifetime shorter than a
 * full axe pass: `copyLink()` swaps the Copy-link button's label to
 * "Link copied ✓" (or "Link in address bar" when the clipboard is refused) and
 * a `setTimeout` puts "Copy link" back 1600ms later. A whole-page scan takes
 * longer than that on a cold run, so it would measure the flashed label only
 * sometimes — and a sometimes-measured assertion is worse than none. Scoped to
 * `.preset-row *` the walk costs a couple of milliseconds, so `driveAllStates`
 * can click, measure, and then assert the flashed label is still on the button,
 * which is what makes the measurement provably of the state it names.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks the whole seven-panel page, and the MAC-diff
     * disclosure alone contributes eight 96-character `.byteblock` rows whose
     * differing bytes are each their own `<mark>` element — every one of those
     * nodes re-walks the same handful of ancestors up to `<body>`. Without the
     * caches the pass re-reads the same computed styles and rects thousands of
     * times, and the gate scans about twenty states per configuration. Nothing
     ...
     * that is every `.byteblock` (`overflow-x: auto`), `.flight-track` and
     * `.flight-code` (both `overflow: hidden`).
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab has two
      // disclosures and both matter here: `.guide-details` ("Or drive it step
      // by step") ships shut on every load, and the MAC-diff `.expert` panel
      // does not exist at all until a handshake has produced a `finished`
      // record. The gate opens each by clicking its <summary> — the route a
      // reader has — rather than setting `.open` from script, which is what the
      // gate this replaces did to every <details> at once, before its one scan.
      ...
      //
      // This lab has no such element today: it pairs its `aria-hidden` ✓/✗/⚠
      // glyphs with a sibling span carrying the same state in plain visible
      // words rather than with a clipped twin. The guard stays because it is
      // the rule for the first `.sr-only` anyone adds — and because that idiom
      // has invented failures in this fleet before, for text nobody can see.
     ...
     *
     * Most of it is glyphs whose meaning is carried in words right beside them,
     * on the same surface, in the same ink — so measuring the words measures the
     * glyph: the `✓`/`✗`/`⚠`/`ℹ` `.chip-icon` in every chip and badge (the
     * label span next to it inherits the identical `color`), the `✂`
     * `.scissors`, the `→` `.wire-arrow`s, the `↓` `.diff-arrow`, and the
     * shared header's two glyph-free `<svg>` marks.
     *
     * ONE subtree is different and is the reason this note exists: the whole
     * `.flight` widget carries `aria-hidden="true"` and contains four pieces of
     * real painted text, including `.flight-code`, whose hex string CHANGES on
     * every run (it visibly shrinks when the attacker cuts the hybrid entry).
     * Neither oracle would ever look at it. Measured by hand against the
     * surfaces it actually lands on:
     *
     *   `.flight-caption`  --text-dim on --surface     10.49:1 dark / 8.18:1 light
     *   `.flight-tick` ×3  --text-dim on --surface-2    9.64:1 dark / 6.81:1 light
     *   `.flight-code`     --text     on --surface     15.89:1 dark / 18.06:1 light
     *
     * All three clear the 4.5:1 their sizes require. `aria-hidden` is
     * defensible on that subtree because it re-illustrates a strip the three
     * `.lane`s and the results region already state in text — but it is a
     * decision that has to be re-checked, not inherited, if anything moves into
     * it that is not also said somewhere a screen reader can reach.
     */
     ...
     *
     * This lab has no SVG figure today, and `ui/dom.ts`'s `h()` builds every
     * node with `createElement` + `append` rather than by interpolating
     * strings, so the usual way this mistake arrives — interpolating a string
     * ARRAY into a template literal, which JS joins with commas and leaves as a
     * run of "," text nodes inside a `<g>` — cannot happen here. The guard
     * stays because it is the rule for any SVG anyone adds, and because a walk
     * that measured such a node would report a 1:1 failure describing ink that
     * was never laid down.
     */
     ...
     *
     * This lab is NOT that lab, and the check is a no-op here today: it sets
     * `html { background-color: #160a0f }` (and `#fbf2f4` under
     * `[data-theme='light']`) with no height constraint, so the root box already
     * spans the document and the ordinary ancestor walk finds it. The composite
     * is kept because "no-op today" is a property of two `height` declarations
     * nobody has written yet, not of the helper — and because the day someone
     * writes them, the alternative is a burst of invented failures for every
     * panel below the first screen, which on a seven-panel page is most of them.
     */
